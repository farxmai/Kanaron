...
export const AttributesCard = ({
  ...
  const points =
    selfAttributes && Object.values(selfAttributes).reduce((acc, value) => acc + value, 0);
        ...
        <TableHead>
          <TableRow>
            <TableCell align='center' colSpan='8'>
              <Typography>{`Атрибуты ${isEdit ? `${points}/${limit}` : ""}`}</Typography>
            </TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {rows.map((row) => (
            <TableRow sx={{ py: 1, border: 0 }} key={row.eng}>
